function compbands(listname,conntype,measure,bandidxs)

for b = 1:length(bandidxs)
    [testdata(:,b),grp,crs,tennis,v2idx] = testmeasure(listname,conntype,measure,bandidxs(b),'randratio','on');
end

testdata = testdata(~v2idx,:);
testdata = testdata(:,2)./testdata(:,1);
