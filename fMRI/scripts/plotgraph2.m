function plotgraph2(subj,bandidx,plotqt)

filepath = '/Users/Ithabi/Documents/results/';

%load([filepath basename 'icohfdr.mat']);
load([filepath 'bigmatfmri.mat']);
matrix = squeeze(bigmat(subj,:,:,:));
chanlocs = bigchanlocs{subj};

matrix = squeeze(matrix(bandidx,:,:));

load('distinfofmri.mat');
degdist = degree(3,:);
wtdist = weight(3,:);
...
tempmat = matrix;
tempmat(tempmat > 0) = 1;
%minfo = modularity_louvain_und(tempmat);
...
set(gcf,'Position',[100 100 figpos(3)*1.5 figpos(4)*1.5]);
h_leg = legend('show');
