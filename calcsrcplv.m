function connfile = calcsrcplv(basename)

loadsubj
loadpaths
brainstormdbpath = '/Users/chennu/Data/brainstorm_db/';
studyname = 'RestingState';
chanlocfile = 'channel_GSN_HydroCel_91_ICBM152.mat';

% Input files
sFiles = [];
datafile = sprintf('%s%s.set',filepath,basename);
...
sFiles = bst_process('CallProcess', 'process_import_channel', sFiles, [], ...
    'channelfile', {sprintf('%s%s',chanlocpath,chanlocfile), 'BST'}, ...    
    'channelalign', 0);

%% copy over head model
headmodelfile = 'headmodel_surf_openmeeg.mat';
copyfile(headmodelfile,sprintf('%s/%s/data/%s/@default_study/',...
    brainstormdbpath,...
    studyname,...
    basename));
ProtocolSubjects = bst_get('ProtocolSubjects');
subjidx = find(strcmp(basename,{ProtocolSubjects.Subject.Name}));
db_reload_subjects(subjidx);
db_reload_conditions(subjidx);
